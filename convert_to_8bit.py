...
def mp3_to_8bit_values(input_file, output_file=None, sample_rate=1000):
    """
    Convert MP3 to 8-bit values (0-255) and save as raw bytes
    ...
    output_file : str, optional
        Path for output file. If not provided, will use input filename with _8bit suffix
    sample_rate : int, optional
        Target sample rate for the audio (default: 1000 Hz)
    ...
    # Save to binary file as raw bytes (no header)
    with open(output_file, 'wb') as f:
        # Write the raw 8-bit values directly as bytes
    ...
    # Save a readable text version with all values
